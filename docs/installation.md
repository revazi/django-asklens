...
For PR10 evidence, the script now creates a disposable SQLite Django project in its temporary workdir, applies the published package migrations (`0001_initial` and `0002_add_admin_query_proxy`) against a synthetic row, replaces the install with the exact local wheel, rechecks migration/plan/check/makemigrations, and verifies the synthetic `SemanticQueryRun` survives with the proxy model and single AskLens table shape. This is migration-state preservation evidence only: not a normal package upgrade, not PostgreSQL migration evidence, and not release evidence.
