...
Evaluation fixtures are deterministic question-to-envelope cases used to verify planner, QueryPlan validation, compiler, core result serialization, and separate presentation behavior without live LLM calls.
...
- Count orders with separate metric presentation metadata.
...
- expected serialized core result and separate presentation shape when relevant.
