...
- [ ] If configuring server-owned `AUDIT_DATABASE_ALIAS`, use one trusted non-empty alias and prove built-in audit writes plus run-detail reads use it without client override or fallback to `default`. Invalid/unavailable configuration must return only fixed safe detail or sink-failure behavior. Lifecycle command `--database` remains independent.
