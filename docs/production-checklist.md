...
- [ ] Expect purge to use an initial primary-key high-water boundary: ordinary later inserts wait for another run, while concurrent updates/deletes may make actual deleted-row counts differ from preview.
- [ ] Review normal Django `pre_delete`/`post_delete` signals and configured cascade/protect/restrict rules. A failed batch rolls back database changes, but external signal-handler effects cannot be rolled back.
- [ ] Confirm lifecycle command output contains no high-water/row IDs or stored content and that both commands act on selected built-in database rows regardless of current `AUDIT_MODE` without invoking custom sinks.
