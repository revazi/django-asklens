...
- [ ] If configuring server-owned `AUDIT_DATABASE_ALIAS`, use one trusted non-empty alias and prove built-in audit writes plus run-detail reads use it without client override or fallback to `default`. Invalid/unavailable configuration must return only the fixed safe `asklens.execute.failed` envelope or sink-failure behavior. Lifecycle command `--database` remains independent.
- [ ] If you use `database` audit mode with read-only replicas/roles, confirm the deployment write-routing/FK topology is reviewed and documented; an explicit audit alias writes the principal as `user_id`, so the host must provide compatible referenced-user data and constraints on that database.
...
- [ ] Account for redaction concurrency and partial progress. Redaction has no primary-key high-water boundary and selects until no eligible content remains, so concurrent changes can alter counts and later eligible inserts can prolong the run. A failing batch rolls back, but rows handled by earlier committed batches remain redacted; rerun preview and reconcile before retrying.
...
- [ ] Confirm the packaged `SemanticQueryRun` admin preserves inherited Django view semantics for authorized list/detail access while denying add/change/delete for every principal, including superusers; `delete_selected` must be unavailable.
- [ ] Treat redaction and purge as AskLens-provided operator workflows, not universal host authorization or mutation controls. Restrict command execution, ORM/direct-database access, custom sinks, and host-defined administration separately.
- [ ] Restrict audit access separately from query access: owner access or global `asklens.view_semanticqueryrun` for cross-user run detail (`is_staff` alone is insufficient), plus independent admin/custom-export policy. Test opaque inaccessible/missing `404` behavior and record administrative access where required.
- [ ] Define redaction at ingestion and display/export boundaries. Run detail reapplies current `AUDIT_INCLUDE_CONTENT` policy to legacy rows and never reflects stored free-form errors, but that is not a substitute for safe storage. Never copy raw rejected input or provider payloads into an audit sink by default.
