...
- **Caller-supplied echo** — question or validated public plan returned to the
  same caller. It can still be privacy-sensitive, but it is not server-owned
  backend/policy leakage.
...
| HTTP/MCP `question` and validated public `plan` | Caller-supplied echo. Privacy-sensitive propagation, but not server-owned backend/policy leakage. Any removal or redaction requires a separate API and privacy-policy decision. |
