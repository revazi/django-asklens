...
Python 3.12+ is required. Installation metadata remains `Django>=5.2,<7.0`, but current CI support evidence is deliberately limited to Django 5.2 LTS, 6.0, and 6.1. The broad compatibility matrix currently exercises Python 3.12/3.13 with each explicit Django 5.2/6.0/6.1 line on SQLite. PostgreSQL semantic/conformance CI covers specific representative stacks on PostgreSQL 15 and 18; it is not a full Cartesian matrix. PG15 is tested with Py3.12/Django 5.2 and Py3.13/Django 6.0; PG18 is tested with Py3.13/Django 6.1. The PostgreSQL 18 reference app also has separate browser/API/MCP smoke evidence on the lock-selected Django 6.1 stack. Record the exact evaluation combination and this limitation rather than implying that every Python/Django/PostgreSQL combination has been tested together.
