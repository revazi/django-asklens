...
def test_capability_help_for_single_facility_user_avoids_multi_facility_examples(
    ...
    resources = {
        resource["name"]: resource for resource in response.data["catalog"]["resources"]
    }
    assert "billing_lines" in resources
    assert "resources" not in response.data["capabilities"]
