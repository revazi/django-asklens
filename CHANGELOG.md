...
- Added optional server-owned `AUDIT_DATABASE_ALIAS=None`; a configured non-empty alias is used explicitly for built-in database audit writes and run-detail reads without client override or fallback to `default`.
- Added preview-by-default `redact_asklens_audit` and `purge_asklens_audit` commands for trusted operators to manually redact content or irreversibly delete older built-in database audit rows in bounded batches.
...
- Run-detail lookup is authorization-filtered before fetch and creates no audit row, preventing the former `403`/`404` existence distinction. Invalid or unavailable configured audit aliases fail closed with fixed public detail or normal audit-sink failure behavior and never reflect alias/database diagnostics or broaden to the default database.
